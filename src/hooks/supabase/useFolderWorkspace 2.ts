...
export function useFolderWorkspace(folderId: string) {
  ...
  // --- sources ---
  const addSource = async (data: Pick<FolderSource, 'type' | 'title' | 'url' | 'content' | 'status' | 'visibility'>) => {
  ...
  // --- memory ---
  const addMemory = async (data: Pick<FolderMemoryItem, 'category' | 'content'>) => {
    if (!user?.id) return null;
  ...
  // --- quick links ---
  const addQuickLink = async (data: Pick<FolderQuickLink, 'title' | 'url' | 'icon'>) => {
    if (!user?.id) return null;
