...
// Standalone type for Supabase knowledge_sources table
// Used for manual metadata entries (separate from Pinecone browsing)
...
const STATUSES = ['pending', 'processed', 'failed', 'archived'];

const EditKnowledgeModal: React.FC<EditKnowledgeModalProps> = ({ open, onOpenChange, source }) => {
  ...
  useEffect(() => {
    if (source) {
      setFormData({
        ...
        bullet_points: source.bullet_points?.join('\n') || '',
        file_date: source.file_date || '',
        box_file_id: source.box_file_id || '',
