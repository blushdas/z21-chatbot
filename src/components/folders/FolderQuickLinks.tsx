...
interface Props {
  ...
  onAdd: (data: Pick<FolderQuickLink, 'title' | 'url' | 'icon' | 'description' | 'category'>) => Promise<unknown>;
  onUpdate: (id: string, data: Partial<Pick<FolderQuickLink, 'title' | 'url' | 'icon' | 'description' | 'category'>>) => Promise<unknown>;
  onDelete: (id: string) => Promise<void>;
}

const FolderQuickLinks: React.FC<Props> = ({ quickLinks, onAdd, onUpdate, onDelete }) => {
  const [open, setOpen] = useState(false);
  const [editingLink, setEditingLink] = useState<FolderQuickLink | null>(null);
  ...
  const openEdit = (link: FolderQuickLink) => {
    setEditingLink(link);
    setTitle(link.title);
    setUrl(link.url);
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    if (editingLink) {
      await onUpdate(editingLink.id, payload);
    } else {
      await onAdd(payload);
    }
    setSaving(false);
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Badge
            variant="outline"
            className="border-[var(--chat-border)] text-[10px] uppercase tracking-wide text-[var(--chat-muted)]"
            title="Quick Links are shortcuts only. Daryle does not read, ingest, or cite them."
          >
            Not used by AI
          </Badge>
        </div>
        <Dialog open={open} onOpenChange={value => { setOpen(value); if (!value) resetForm(); }}>
          ...
          <DialogContent className="bg-[var(--chat-card)] border-[var(--chat-border)] text-[var(--chat-text)]">
            <DialogHeader><DialogTitle>{editingLink ? 'Edit Quick Link' : 'Add Quick Link'}</DialogTitle></DialogHeader>
            <div className="space-y-3 pt-2">
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button size="sm" onClick={handleSubmit} disabled={saving || !title.trim() || !url.trim()}>
                  {saving ? 'Saving...' : editingLink ? 'Save Link' : 'Add Link'}
                </Button>
              </div>
            </div>
          </DialogContent>
        ...
        {quickLinks.map(link => {
          ...
          return (
            <div key={link.id} className="group relative flex flex-col gap-2 p-3 rounded-lg bg-[var(--chat-bg)] border border-[var(--chat-border)] hover:border-[var(--chat-muted)] transition-colors">
              <a href={link.url} target="_blank" rel="noreferrer" className="flex items-start gap-2 flex-1 pr-10">
                ...
                <div className="flex-1 min-w-0 space-y-0.5">
                  ...
                  {link.description && <p className="text-xs text-[var(--chat-muted)] line-clamp-2">{link.description}</p>}
              ...
              <div className="absolute top-1 right-1 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <Button
                  variant="ghost"
                  size="sm"
                  className="h-5 w-5 p-0 text-[var(--chat-muted)]"
